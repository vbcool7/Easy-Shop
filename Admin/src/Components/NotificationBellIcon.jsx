...
import { Bell, BellRing, Check, CheckCheck, Package, Star, AlertTriangle, Wallet, X, Users, UserPlus, ShoppingBag } from 'lucide-react';

import { io } from "socket.io-client";
import { useGetAdminNotification, useMarkAllAdminReadNotification, useMarkSingleReadNotification } from '../hooks/useNotification';
import { useTranslation } from 'react-i18next';

const TYPE_CONFIG = {
    NEW_VENDOR: { icon: Users, color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-500/10' },
    NEW_USER: { icon: UserPlus, color: 'text-teal-500', bg: 'bg-teal-50 dark:bg-teal-500/10' },
    NEW_ORDER: { icon: Package, color: 'text-pink-500', bg: 'bg-pink-50 dark:bg-pink-500/10' },
    NEW_PRODUCT: { icon: ShoppingBag, color: 'text-violet-500', bg: 'bg-violet-50 dark:bg-violet-500/10' },
    PAYOUT_REQUEST: { icon: Wallet, color: 'text-green-500', bg: 'bg-green-50 dark:bg-green-500/10' },
    LOW_STOCK: { icon: AlertTriangle, color: 'text-orange-500', bg: 'bg-orange-50 dark:bg-orange-500/10' },
};

const getRedirectPage = (type) => {
    switch (type) {
        case 'NEW_VENDOR': return 'manage-vendors';
        case 'NEW_USER': return 'customers';
        case 'NEW_ORDER': return 'platform-orders';
        case 'NEW_PRODUCT': return 'all-products';
...
function NotificationBellIcon({ setCurrentPage }) {
    ...
    const { data, isLoading, refetch } = useGetAdminNotification({ pageNum: page });
    const { mutate: markAll } = useMarkAllAdminReadNotification();
    ...
    const timeAgo = (dateStr) => {
        ...
        if (mins < 1) return t('adminNotifications.justNow');
        if (mins < 60) return t('adminNotifications.minsAgo', { count: mins });
        const hrs = Math.floor(mins / 60);
        if (hrs < 24) return t('adminNotifications.hrsAgo', { count: hrs });
        const days = Math.floor(hrs / 24);
        return t('adminNotifications.daysAgo', { count: days });
    };

    useEffect(() => {
        const socket = io(import.meta.env.VITE_SOCKET_URL);

        socket.emit("join_notifications", { role: "admin" });
                                ...
                                <div className="flex min-w-0 flex-wrap items-center gap-2">
                                    ...
                                    {unreadCount > 0 && (
                                        <span className="shrink-0 rounded-full bg-pink-50 px-1.5 py-0.5 text-[10px] font-semibold leading-none text-pink-500 dark:bg-pink-500/10">
                                            {t('adminNotifications.newBadge', { count: unreadCount })}
                                        </span>
                                    )}
                                </div>
